<template>
  <div>
    <div class="form-check" v-for="(option, index) in propOptions">
      ...
      <input 
        type="radio" class="form-check-input" :name="'radio-'+uid" :id="'checkbox-'+uid+'-'+index" :checked="propMode != 'edit' && isChecked(option)" 
...
function selectionChanged(option) {  
  propValue.value.splice(0, propValue.value.length);
